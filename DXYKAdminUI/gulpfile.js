...
gulp.task('clearRelease', function(cb) {
  return del(['static/json/*', releaseDir], cb);
});
...
gulp.task('release', function(){ //命令：gulp && gulp release
  ...
  .pipe(gulp.dest(releaseDir + '/static/json'));
  ...
  .pipe(gulp.dest(releaseDir + '/static'));
  ...
  .pipe(gulp.dest('./static/layui'))
  .pipe(gulp.dest(releaseDir + '/static/layui'))
});
