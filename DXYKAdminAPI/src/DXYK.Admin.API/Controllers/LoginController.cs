...
namespace DXYK.Admin.API.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        /// </summary>
        private readonly string _captchaCodeSessionNameForRegister = "DXYKCaptchaCodeForRegister";

        /// <summary>
        /// 微信生成码 在内存中默认保存Key 登录时使用
        /// </summary>
        private readonly string _wechatCodeSessionNameForLogin = "WechatCodeSessionNameForLogin";
        */
        ///<summary>
        /// 用户信息UserService
        ///</summary>
        ...
        public ResponseMessage<string> LoginIn([FromBody]UserLoginDto reqLogin)
        {
            ...
            try
            {
                if (!ValidateCaptchaCode(reqLogin.vercode, _captchaCodeSessionNameFroLogin))
                {
                ...
                List<Menu> menu = new List<Menu>() { new Menu() { id = 111 } };
                if (menuSaveType == "Redis")
                {
                    RedisHelper.Set(AppSettingKeyHelper.ADMINMENUACTION + "_" + user.id, menu);
                    //RedisHelper.Set(AppSettingKeyHelper.NOWSITE, site);
                }
                else
                {
                    //MemoryCacheService.Default.SetCache(AppSettingKeyHelper.NOWSITE, site);
                    MemoryCacheService.Default.SetCache(AppSettingKeyHelper.ADMINMENUACTION + "_" + user.id, menu, 600);
                }
        ...
        {
            List<Permission> list = _authorizeService.GetPermission("GXBBWGKGLJ", 1);
            return "";
        }
