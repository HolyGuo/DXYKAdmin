<template>
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <!-- 新增 -->
      <div v-if="checkPermission(['Admin','Role_Manage','Role_Add'])" style="display: inline-block;margin: 0px 2px;">
        <el-button
          class="filter-item"
          size="mini"
          type="primary"
      ...
      <el-col :xs="24" :sm="24" :md="16" :lg="16" :xl="17" style="margin-bottom: 10px">
        <el-card class="box-card" shadow="never">
          ...
          <el-table v-loading="loading" :data="data" highlight-current-row size="small" style="width: 100%;" @current-change="handleCurrentChange">
            ...
            <el-table-column v-if="checkPermission(['Admin','Role_Manage','Role_Delete','Role_Update'])" label="操作" width="130px" align="center" fixed="right">
              <template slot-scope="scope">
                <el-button v-if="checkPermission(['Admin','Role_Manage','Role_Update'])" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)"/>
                <el-popover
                  v-if="checkPermission(['Admin','Role_Manage','Role_Delete'])"
                  :ref="scope.row.id"
                  placement="top"
                  width="180">
                  <p>确定删除本条数据吗？</p>
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      ...
      this.url = 'api/SysAppRole/GetAllByName'
      // const sort = 'level,asc'
      const query = this.query
      const value = query.value
      this.params = { page: this.page + 1, limit: this.size }
    ...
    savePermission() {
      ...
      const role = { roleid: this.currentId, type: 2, objs: [] }
    ...
    edit(data) {
      ...
      _this.deptIds = []
      _this.form = { id: data.id, role_code: data.name, remark: data.remark, depts: data.depts, dataScope: data.dataScope, level: data.level }
